...
TEST_CASE( "lhs = rhs", "[Counter]" ) 
{
  ...
  C.insert(20); C.insert(21); C.insert(22); C.insert(22);
    
  REQUIRE(C.empty() == false);
  REQUIRE(C.size() == 3);
  REQUIRE(C2.empty() == true);
  REQUIRE(C2.size() == 0);
...
TEST_CASE( "Fill with random numbers Counter<int>", "[Counter]" ) 
{
  srand(time(NULL));
  Counter<int> C;
